...
FW_LIB_DECLARE(uint8_t) fw_lib_txt_msg_build_command(const uint32_t device_id, const uint8_t message_id, fw_lib_msg_arg_t* args, uint8_t* packet_buf)
{
  ...
  if (packet_buf != NULL)
  {
    ...
    case FW_LIB_MSG_ID_READ_HW_VERSION:
      // RHVER device_id\n
      // ex) RHVER 1\n
      len = sprintf((char*)packet_buf, "%s %d%c", fw_lib_txt_msg_get_message_name(message_id), device_id, FW_LIB_TXT_MSG_TAIL);
      break;
    ...
    case FW_LIB_MSG_ID_READ_GPIO:
      ...
      // ex) RGPIO 1,2\n
      len = sprintf((char*)packet_buf, "%s %d,%d%c", fw_lib_txt_msg_get_message_name(message_id), device_id, args[0].value.uint8_value, FW_LIB_TXT_MSG_TAIL);
      break;

    case FW_LIB_MSG_ID_WRITE_GPIO:
      // WGPIO device_id,button_num,button_value\n
      // ex) WGPIO 1,1,1\n
      len = sprintf((char*)packet_buf, "%s %d,%d,%d%c", fw_lib_txt_msg_get_message_name(message_id), device_id, args[0].value.uint8_value, args[1].value.uint8_value, FW_LIB_TXT_MSG_TAIL);
      break;
    }
  }
...
FW_LIB_DECLARE(uint8_t) fw_lib_txt_msg_build_response(const uint32_t device_id, const uint8_t message_id, fw_lib_msg_arg_t* args, uint8_t error, uint8_t* packet_buf)
{
  ...
  if (packet_buf != NULL)
  {
    ...
    case FW_LIB_MSG_ID_READ_HW_VERSION:
      ...
      // ex) RHVER 1,0,1.2.3\n
      len = sprintf((char*)packet_buf, "%s %d,%d,%s%c", fw_lib_txt_msg_get_message_name(message_id), device_id, error, args->value.string_value, FW_LIB_TXT_MSG_TAIL);
      break;
    ...
    case FW_LIB_MSG_ID_READ_GPIO:
      ...
      // ex) RGPIO 1,0,2,0\n
      len = sprintf((char*)packet_buf, "%s %d,%d,%d,%d%c", fw_lib_txt_msg_get_message_name(message_id), device_id, error, args[0].value.uint8_value, args[1].value.uint8_value, FW_LIB_TXT_MSG_TAIL);
      break;

    case FW_LIB_MSG_ID_WRITE_GPIO:
      // WGPIO device_id,error\n
      // ex) WGPIO 1,0\n
      len = sprintf((char*)packet_buf, "%s %d,%d%c", fw_lib_txt_msg_get_message_name(message_id), device_id, error, FW_LIB_TXT_MSG_TAIL);
      break;
    }
  }
